...
1. 获取数据（每一个数据为一个m维列向量）。
2. 求出数据平均值 ，并用原数据减去均值得到。
3. 计算协方差矩阵
4. 计算协方差矩阵的特征值 与特征向量 （特征值与特征向量成对出现）。
5. 由大到小依次排列特征值得 ，对应特征向量为 。特征向量代表了原数据的分布方向，其对应的特征值越大，则该向量越重要（即为主元）；其对应的特征值越小，则该向量越次要。
